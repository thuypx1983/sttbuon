 ...
 */
function list_mag_wp_custom_colors_css() {

// Main Color (yellow)
$list_mag_wp_main_color1 = get_theme_mod('list_mag_wp_main_color1');
if (!empty($list_mag_wp_main_color1)) {
    // BG Color
    echo esc_html('ul.meta-icons-home li.trending-lm, .trending-lm .tooltiptext, .sticky-lm .tooltiptext, ul.article_list li ul.meta-icons-home li.trending-lm { background-color: '. $list_mag_wp_main_color1 .' !important;} ');

    // Border
    echo esc_html('.trending-lm .tooltiptext::after, .sticky-lm .tooltiptext::after { border-color: transparent transparent '. $list_mag_wp_main_color1 .' !important transparent;} ');    
}
// Main Color (green)
$list_mag_wp_main_color2 = get_theme_mod('list_mag_wp_main_color2');
if (!empty($list_mag_wp_main_color2)) {
    // BG Color
    echo esc_html('ul.meta-icons-home li.sticky-lm, .sticky-lm .tooltiptext, .listbtn-category, ul.menu-left li a:hover, .single-category a { background-color: '. $list_mag_wp_main_color2 .' !important;} ');
    // Border
...
if (!empty($list_mag_wp_main_color3)) {
    // BG Color
    echo esc_html('#infscr-loading, #searchform2 .buttonicon, .my-paginated-posts span, #tags-wrap, #back-top span { background-color: '. $list_mag_wp_main_color3 .' !important;} ');
    // Border
