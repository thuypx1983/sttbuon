...
class list_mag_wp_posttags extends WP_Widget {
     function list_mag_wp_posttags() {
      $widget_ops = array('description' => esc_html__('Posts by Tags', 'list-mag-wp'));
        parent::__construct(false, $name = ''. esc_html__('Custom: Posts by Tags', 'list-mag-wp') .'',$widget_ops);  
    }
...
<?php // The Query
$list_mag_wp_widget_recent = new WP_Query(array('post_type' => 'post',  'ignore_sticky_posts' => 1,'tag' => esc_attr($arttag), 'posts_per_page' => esc_attr($number) )); // number to display more / less ?>
<?php while ($list_mag_wp_widget_recent->have_posts()) : $list_mag_wp_widget_recent->the_post(); ?> 
...
?>
        ...
        <p>
         ...
         <p>
          ...
          <p>
          <label for="<?php echo $this->get_field_id('arttag'); ?>"><?php esc_html_e( 'Tag:', 'list-mag-wp' ); ?></label>      
...
<?php  } } 
add_action('widgets_init', create_function('', 'return register_widget("list_mag_wp_posttags");')); // register widget
?>