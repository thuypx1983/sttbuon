...
class list_mag_wp_recentposts extends WP_Widget {
     function list_mag_wp_recentposts() {
      $widget_ops = array('description' => esc_html__('Your site\'s most recent Posts.', 'list-mag-wp'));
        parent::__construct(false, $name = ''. esc_html__('Custom: Recent Posts', 'list-mag-wp') .'',$widget_ops); 
     }
...
<?php $list_mag_wp_latest = new WP_Query(array('post_type' => 'post', 'ignore_sticky_posts' => 1, 'posts_per_page' => esc_attr($number) )); // number to display more / less ?>
...
<?php  } } 
add_action('widgets_init', create_function('', 'return register_widget("list_mag_wp_recentposts");')); // register widget
?>