...
class list_mag_wp_likedposts extends WP_Widget {
     function list_mag_wp_likedposts() {
      $widget_ops = array('description' => esc_html__('Top Posts by likes.', 'list-mag-wp'));
        parent::__construct(false, $name = ''. esc_html__('Custom: Top Posts by likes.', 'list-mag-wp') .'',$widget_ops); 
     }
...
<?php // The Query
$list_mag_wp_toplikes = new WP_Query(array('meta_key' => '_thumbs_rating_up', 'ignore_sticky_posts' => 1, 'orderby' => 'meta_value_num', 'order' => 'DESC', 'posts_per_page' => esc_attr($number) )); // number to display more / less ?>
<?php while ($list_mag_wp_toplikes->have_posts()) : $list_mag_wp_toplikes->the_post(); ?> 
...
<?php  } } 
add_action('widgets_init', create_function('', 'return register_widget("list_mag_wp_likedposts");')); // register widget
?>