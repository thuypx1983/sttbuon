...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		global $smof_output, $smof_details, $kickcube_wp_data;
		if (empty($options))
			return;
		if (empty($kickcube_wp_data))
			$kickcube_wp_data = of_get_options();
		$data = $kickcube_wp_data;
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($kickcube_wp_data) || !empty($data)){
			
				if (array_key_exists('id', $value) && !isset($kickcube_wp_data[$value['id']])) {
					$kickcube_wp_data[$value['id']] = $value['std'];
					if ($value['type'] == "checkbox" && $value['std'] == 0) {
						$kickcube_wp_data[$value['id']] = 0;
					} else {
						$update_data = true;
					}
				}
				if (array_key_exists('id', $value) && !isset($smof_details[$value['id']])) {
					$smof_details[$value['id']] = $kickcube_wp_data[$value['id']];
				}
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
				...
				if (array_key_exists("fold",$value)) {
					if (isset($kickcube_wp_data[$value['fold']]) && $kickcube_wp_data[$value['fold']]) {
						$fold="f_".$value['fold']." ";
					} else {
			...
			//if (!isset($kickcube_wp_data[$value['id']]) && $value['type'] != "heading")
			//	continue;
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = stripslashes($kickcube_wp_data[$value['id']]);
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {
						...
						if (!is_numeric($select_ID))
							$theValue = $select_ID;
						$output .= '<option id="' . $select_ID . '" value="'.$theValue.'" ' . selected($kickcube_wp_data[$value['id']], $theValue, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes($kickcube_wp_data[$value['id']]);			
						$output .= '<textarea class="of-input" name="'.$value['id'].'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';		
				break;
				
				//radiobox option
				case "radio":
					$checked = (isset($kickcube_wp_data[$value['id']])) ? checked($kickcube_wp_data[$value['id']], $option, false) : '';
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($kickcube_wp_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($kickcube_wp_data[$value['id']])) {
						$kickcube_wp_data[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($kickcube_wp_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					(isset($kickcube_wp_data[$value['id']]))? $multi_stored = $kickcube_wp_data[$value['id']] : $multi_stored="";
				...
				case "color":
					...
					$output .= '<input name="' . $value['id'] . '" id="' . $value['id'] . '" class="of-color"  type="text" value="' . $kickcube_wp_data[$value['id']] . '"' . $default_color .' />';
				...
				case 'typography':
					...
					$typography_stored = isset($kickcube_wp_data[$value['id']]) ? $kickcube_wp_data[$value['id']] : $value['std'];
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $kickcube_wp_data[$value['id']];
				...
				case 'images':
					...
					$select_value = (isset($kickcube_wp_data[$value['id']])) ? $kickcube_wp_data[$value['id']] : '';
				...
				case 'slider':
					...
					$slides = $kickcube_wp_data[$value['id']];
					$count = count($slides);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = isset($kickcube_wp_data[$value['id']]) && !empty($kickcube_wp_data[$value['id']]) ? $kickcube_wp_data[$value['id']] : '';
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					// $output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($kickcube_wp_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
				...
				case 'select_google_font':
					...
					foreach ($value['options'] as $select_key => $option) {
						$output .= '<option value="'.$select_key.'" ' . selected((isset($kickcube_wp_data[$value['id']]))? $kickcube_wp_data[$value['id']] : "", $option, false) . ' />'.$option.'</option>';
					} 
					$output .= '</select></div>';
					...
					$hide = " hide";
					if ($kickcube_wp_data[$value['id']] != "none" && $kickcube_wp_data[$value['id']] != "")
						$hide = "";
				...
				case 'sliderui':
					...
					$s_val  = stripslashes($kickcube_wp_data[$value['id']]);
				...
				case 'switch':
					...
					//Get selected
					if ($kickcube_wp_data[$value['id']] == 1){
						$cb_enabled = ' selected';
						$cb_disabled = '';
					}else{
						$cb_enabled = '';
					...
					$output .= '<p class="switch-options">';
						...
						$output .= '<input type="hidden" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="0"/>';
						$output .= '<input type="checkbox" id="'.$value['id'].'" class="'.$fold.'checkbox of-input main_checkbox" name="'.$value['id'].'"  value="1" '. checked($kickcube_wp_data[$value['id']], 1, false) .' />';
				...
				case "media":
					...
					$u_val = '';
					if($kickcube_wp_data[$value['id']]){
						$u_val = stripslashes($kickcube_wp_data[$value['id']]);
					}
		...
		if ($update_data == true) {
			of_save_options($kickcube_wp_data);
		}
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		$uploader = '';
		$upload = "";
		if (isset($kickcube_wp_data[$id]))
	    	$upload = $kickcube_wp_data[$id];
		$hide = '';
